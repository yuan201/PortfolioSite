...
class NewPortfolioTest(StaticLiveServerTestCase):
    ...
    def test_create_new_portfolio(self):
        ...
        # On the front page, he nothing since no portfolio has been created
        # The title of the page is simply called portfolio
        self.assertIn('Portfolio', self.browser.title)

        # Follow the link for creating new portfolio
        link = self.browser.find_element_by_link_text('New Portfolio')
        link.click()

        # Enter portfolio name "Value" and description "Simple value portfolio"
        inputbox = self.browser.find_element_by_id('id_name')
        inputbox.send_keys('Value')

        inputbox = self.browser.find_element_by_id('id_description')
        inputbox.send_keys('Simple value portfolio')
